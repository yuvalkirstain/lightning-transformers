...
from lightning_transformers.task.nlp.seq2seq.single_ref.config import SingleRefDataConfig


class SingleRefDataModule(Seq2SeqDataModule):
    cfg: SingleRefDataConfig

    def preprocess_dataset(self, example, idx):
        return example

    def process_data(self, dataset: Dataset, stage: Optional[str] = None) -> Dataset:
        dataset = dataset.map(self.preprocess_dataset, with_indices=True)
